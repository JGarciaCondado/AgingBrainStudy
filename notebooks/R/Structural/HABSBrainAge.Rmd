...
```{r}
regression_analysis(data, 'ptau', 'delta', c('SEX', 'YrsEd', 'E4_Status', 'PIB_FS_SUVR_Group', 'age'))

regression_analysis(data, 'ptau', 'delta', c('nfl', 'gfap'))
```

```{r}
fig <- scatter_plot <- ggplot(data, aes(x = delta, y = ptau)) +
    geom_point() +
    geom_smooth(method = 'lm', formula = y ~ x) +
    stat_cor(method = 'pearson') +
    labs(x = 'Delta (years)',
...
regression_analysis(data, 'nfl', 'delta', c('ptau', 'gfap'))
