...
Demographics split by cohort: A4/LEARN, HABS, ADNI CU and ADNI MCI
...
```{r}
# Follow up time minimum and maximum by cohort
follow_up_summary <- data_all %>%
  group_by(cohort) %>%
  summarise(
    min_follow_up = min(follow_up_time, na.rm = TRUE),
    max_follow_up = max(follow_up_time, na.rm = TRUE)
  )
follow_up_summary
```
...
Further breakdown between primary and exploratory analysis. Primary analysis are all subjects with an MRI, longitudinal PACC and also AB-PET. Exploratory analysis are all subjects with previous measures and als pTau217 and Tau-PET.
...
data_combined <- bind_rows(data_primary, data_exploratory)
...
data_combined$analysis <- factor(data_combined$analysis, levels = c("primary", "exploratory"))
...
Breakdown in each cohort of each of the individual measures of: AB-PET, pTau217 and Tau-PET.
...
# Count how many have all variables
