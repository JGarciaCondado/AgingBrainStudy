...
simulate_longitudinal_data <- function(
  ...
  data <- data.frame(
    ...
    randint = randeff[, 1],
    randsl = randeff[, 2],
    txt = rep(c(0, 1), length.out=numsubj)
  )
...
```{r}
# Create simulate trial function
simulate_trial <- function(n) {
  ...
  return(time_txt_pval)
}
```

```{r}
estimate_power_parallel <- function(n, reps = 100, alpha = 0.05, cores = detectCores() - 1) {
  # Create the cluster
  cl <- makeCluster(cores)
  
  # Export necessary functions and objects to the cluster
  clusterExport(cl, c("simulate_trial", "simulate_longitudinal_data", 
                      "t", "intercept", "slope", "random_intercept_var", 
                      "random_slope_var", "corr_intercept_slope", 
                      "random_residual_var", "effect"))
...
```{r}
simulate_trial(2000)
estimate_power_parallel(2500, 100)
```

```{r}
find_sample_size_parallel <- function(n_min = 1000, n_max = 4000, power_target = 0.9, 
                                      alpha = 0.05, reps = 100, nearest = 50, 
                                      cores = detectCores() - 1) {
  ...
  while (n_min < n_max) {
    ...
    power_est <- estimate_power_parallel(n_mid, reps, alpha, cores)
    print(power_est)
