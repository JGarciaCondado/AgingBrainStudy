...
method = 'edland'
outcome = 'PACC'
t_visits_a4 = seq(0, 4, 0.5)

# Standard trail
lme_ptau_all <-lmer(as.formula(paste(outcome, "~ time_mri + LMSTORY + Cumulative_Dose_Scaled + time_mri*cohort +  (1 + time_mri | ID)")), data = data_a4_ptau217)
pwr_ptau_all <- lmmpower(lme_ptau_all, pct.change = treatment_eff, t = t_visits_a4, power = power, sig.level = sig_level, n = n, method = method)
...
cat(sprintf("%-12s %-12s %-12s %-15s %-12s\n", 
            "pTau217", "Delta", "Available", "Trial Required", "Reduction"))
cat(sprintf("%-12s %-12s %-12s %-15s %-12s\n", 
            "Threshold", "Threshold", "Participants", "Sample Size", "(%)"))
cat(paste(rep("-", 80), collapse=""), "\n")
...
for (ptau_quantile in ptau217_quantiles) {
  for (delta_quantile in delta_quantiles) {
    ...
    # Model
    lme_ptau <- lmer(as.formula(paste(outcome, "~time_mri + LMSTORY + (1 + time_mri | ID)")), data = data_filtered)
    
    # Calculate power / sample size
    pwr_ptau <- lmmpower(lme_ptau, pct.change = treatment_eff, t = t_visits_a4, power = power, sig.level = sig_level, n = n, method = method)
    ...
    cat(sprintf("%-12.2f %-12.2f %-12d %-15.0f %-12.1f\n", 
                ptau_quantile, delta_quantile, n_participants, n_a4_ptau, n_percentage))
  }
}
...
ct_plot <- function(panel_data, title) {
  ...
  p <- ggplot() +
    ...
    geom_text(aes(x = 1, y = 100, label = "100%"), 
              vjust = -1.5, size = 3.5, color = "gray50") +
...
legend_plot <- ggplot() +
  ...
  geom_text(aes(x = 4, y = 1), label = "BrainAge delta quartiles — Q2-Q4   - - Q3-Q4   ··· Q4", size = 3, hjust = 0) +
  xlim(0, 8) + ylim(0, 1.5) +
  theme_void()
