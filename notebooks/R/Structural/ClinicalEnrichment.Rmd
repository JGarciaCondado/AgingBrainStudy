...
# Plot model
adni_opt_p <- plot(ggpredict(lmm_adni_opt, terms = c("time_mri [all]"))) +
  theme_minimal() +
  scale_x_continuous(limits = c(0, max(data$time_mri))) + 
  scale_y_continuous(limits = c(0, 4.5)) +
  labs(
...
ggsave(paste(path, 'lme_adni_opt.png'), adni_opt_p, 
       width = 70, height = 70, units = "mm", 
       dpi = 500,   # Higher resolution
       scale = 1)
```
...
cat("Number of participants required for clinical trial: ", n_adni_opt, "\n")

# Reduction percentage
reduction_optimum <- (n_adni_all - n_adni_opt) / n_adni_all * 100
cat("Reduction in number of participants required: ", reduction_optimum, "%\n")
```
...
cat("MRI pass rate: ", mri_pass_rate_opt, "\n")
...
cat("Total cost of the trial: ", cost_adni_opt$total_cost, "\n")
cat("Screening cost: ", cost_adni_opt$screening_cost, "\n")
cat("Participants to screen: ", cost_adni_opt$participants_to_screen, "\n")
cat("Treatment cost: ", cost_adni_opt$treatment_cost, "\n")
```
...
```{r}
# Create model
lmm_a4_opt <-lmer(as.formula(paste(outcome, "~time_mri + LMSTORY + (1 + time_mri | ID)")), data = data_a4_opt)

# Plot model
a4_opt_p <- plot(ggpredict(lmm_a4_opt, terms = c("time_mri [all]"))) +
  theme_minimal() +
  scale_x_continuous(limits = c(0, max(data_placebo$time_mri))) + 
  scale_y_continuous(limits = c(-1, 1)) +
  labs(
...
ggsave(paste(path, 'lme_a4_opt.png'), a4_opt_p, 
       width = 70, height = 70, units = "mm", 
       dpi = 500,   # Higher resolution
       scale = 1)
```
...
```{r}
# calculate power / sample size using longpower package
pwr_opt_a4 <- lmmpower(lmm_a4_opt, pct.change = treatment_eff, t = t_visits_a4, power = power, sig.level = sig_level, n = n, method = method)

# Print number of subjects
n_a4_opt <- pwr_opt_a4$N
cat("Number of participants required for clinical trial: ", n_a4_opt, "\n")
...
# Screen fails for MRI
mri_pass_rate_opt = n_a4_participants_opt / n_a4_participants
cat("MRI pass rate: ", mri_pass_rate_opt, "\n")
```
