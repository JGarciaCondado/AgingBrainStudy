...
ab_levels <- c(
  sprintf("Aβ-PET = \n %.2f (-1 SD)", habs_ab_mean - habs_ab_sd),
  sprintf("Aβ-PET = \n %.2f (Mean)", habs_ab_mean),
  sprintf("Aβ-PET = \n %.2f (+1 SD) ", habs_ab_mean + habs_ab_sd)
)
...
habs_ptau_effects <- data.frame(
    ...
    ptau.n = lme_habs_ptau$dims$ngrps
  )
...
adni_cn_delta_effects <- data.frame(
    ...
    delta.n = lme_adni_cn$dims$ngrps
  )
...
adni_cn_ab_effects <- data.frame(
    ...
    ab.n = lme_adni_cn_ab$dims$ngrps
  )
...
ab_levels <- c(
  sprintf("Aβ-PET = \n %.2f (-1 SD)", adni_cn_ab_mean - adni_cn_ab_sd),
  sprintf("Aβ-PET = \n %.2f (Mean)", adni_cn_ab_mean),
  sprintf("Aβ-PET = \n %.2f (+1 SD)", adni_cn_ab_mean + adni_cn_ab_sd)
)
...
adni_cn_ptau_effects <- data.frame(
    ...
    ptau.n = lme_adni_cn_ptau$dims$ngrps
  )
...
adni_cn_tau_effects <- data.frame(
    ...
    tau.n = lme_adni_cn_tau$dims$ngrps
  )
...
adni_mci_delta_effects <- data.frame(
    ...
    delta.n = lme_adni_mci$dims$ngrps
  )
...
adni_mci_ab_effects <- data.frame(
    ...
    ab.n = lme_adni_mci_ab$dims$ngrps
  )
...
ab_levels <- c(
  sprintf("Aβ-PET = \n %.2f (-1 SD)", adni_mci_ab_mean - adni_mci_ab_sd),
  sprintf("Aβ-PET = \n %.2f (Mean)", adni_mci_ab_mean),
  sprintf("Aβ-PET = \n %.2f (+1 SD)", adni_mci_ab_mean + adni_mci_ab_sd)
)
...
adni_mci_ptau_effects <- data.frame(
    ...
    ptau.n = lme_adni_mci_ptau$dims$ngrps
  )
...
adni_mci_tau_effects <- data.frame(
    ...
    tau.n = lme_adni_mci_tau$dims$ngrps
  )
...
forest_plot <- ggplot(delta_interaction_results, aes(x = delta.estimate, y = model)) +
  ...
  geom_text(aes(x = delta.estimate, 
              label = sprintf("p = %.2g, N = %d", delta.p_value, delta.n)),
          vjust = 2.5, size = 7 / .pt,
          inherit.aes = TRUE,
          check_overlap = TRUE) +
...
forest_plot <- ggplot(ab_interaction_results, aes(x = ab.estimate, y = model)) +
  ...
  geom_text(aes(x = ab.estimate, 
              label = sprintf("p = %.2g, N = %d", ab.p_value, ab.n)),
          vjust = 2.5, size = 7 / .pt,
          inherit.aes = TRUE,
          check_overlap = TRUE) +
...
forest_plot <- ggplot(ptau_interaction_results, aes(x = ptau.estimate, y = model)) +
  ...
  geom_text(aes(x = ptau.estimate, 
              label = sprintf("p = %.2g, N = %d", ptau.p_value, ptau.n)),
          vjust = 2.5, size = 7 / .pt,
          inherit.aes = TRUE,
          check_overlap = TRUE) +
  scale_x_continuous(limits = c(-0.5, 0.2))
...
forest_plot <- ggplot(tau_interaction_results, aes(x = tau.estimate, y = model)) +
  ...
  geom_text(aes(x = tau.estimate, 
              label = sprintf("p = %.2g, N = %d", tau.p_value, tau.n)),
          vjust = 2.5, size = 7 / .pt,
          inherit.aes = TRUE,
          check_overlap = TRUE) +
  scale_x_continuous(limits = c(-0.5, 0.2))
