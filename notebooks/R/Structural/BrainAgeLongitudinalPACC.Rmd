...
```{r}
threshold_A4 = results_A4$optimal_threshold
lmm_results_A4_pacc_old <- lme(PACC ~ pacc_time + Cumulative_Dose_Scaled + QSVERSION_PACC + pacc_time*Cohort, data = subset(data_A4, data_A4$delta_all > threshold_A4), random = ~ pacc_time|BID, na.action = 'na.omit', method = c('ML'), control = lmeControl(opt = "optim", maxIter = 500, msMaxIter = 1000));
...
```{r}
threshold_habs = results_habs$optimal_threshold
lmm_results_habs_pacc_old <- lme(NP_PACC_PACC5 ~ time_from_bl, data = subset(data_habs, data_habs$delta_all > threshold_habs), random = ~time_from_bl|SubjIDshort, na.action = 'na.omit', method = c('ML'));
...
```{r}
predict_pacc <- function(model, subject_id, time_point) {
  # Create new data frame for prediction
  new_data <- data.frame(
    SubjIDshort = subject_id,
    time_from_bl = time_point
  )
  
  # Get prediction
  predicted_value <- predict(model, newdata = new_data, allow.new.levels = FALSE)
  
  return(data.frame(
    SubjIDshort = subject_id,
    time_from_bl = time_point,
    predicted_pacc = predicted_value
  ))
}

# Predict for each subject PACC at time 2
predicted_pacc <- sapply(unique(data_habs$SubjIDshort), function(id) predict_pacc(lmm_results_habs_pacc, id, 4))

# Transpose dataframe
predicted_pacc <- as.data.frame(t(predicted_pacc))
rownames(predicted_pacc) <- NULL

# Assign treatment group randomly in predicted_pacc
predicted_pacc$treatment <- as.numeric(sample(c(0, 1), nrow(predicted_pacc), replace = TRUE))
# QUESTION: Do we do 10% of mean or from each subject
treatment_effect = 0.1

# Ensure all variables are numeric
predicted_pacc$predicted_pacc <- as.numeric(predicted_pacc$predicted_pacc)
predicted_pacc$treatment <- as.numeric(predicted_pacc$treatment)
predicted_pacc$predicted_pacc <- as.numeric(predicted_pacc$predicted_pacc)
predicted_pacc$pacc_treatment <- predicted_pacc$predicted_pacc + abs(predicted_pacc$treatment*treatment_effect*predicted_pacc$predicted_pacc)

# Calculate cohen's d between treatment and non treatment
calculate_cohens_d <- function(data, pacc_column, treatment_column) {
  # Extract the PACC values for each treatment group
  group1 <- data[data[[treatment_column]] == levels(factor(data[[treatment_column]]))[1], pacc_column]
  group2 <- data[data[[treatment_column]] == levels(factor(data[[treatment_column]]))[2], pacc_column]
...
cohen_d <- calculate_cohens_d(predicted_pacc, 'pacc_treatment', 'treatment')
pwr.t.test(d = cohen_d$estimate, power = 0.8, sig.level = 0.05, type = "two.sample", alternative = "two.sided")
```


```{r}
# Assign to each subject to be sim_treatment either 0 or 1 using a random number
treatment_assignment <- data_habs %>%
  distinct(SubjIDshort) %>%           # Get unique subjects
  mutate(treatment = sample(c(0, 1), n(), replace = TRUE))  # Randomly assign treatment
data_treatment <- data_habs %>% 
  left_join(treatment_assignment, by = 'SubjIDshort')
# Simulate 10% treatment effect on PACC
data_treatment <- data_treatment %>% 
  mutate(pacc_treatment = NP_PACC_PACC5 + abs(treatment*0.1*NP_PACC_PACC5))

lmm_results_habs_pacc <- lmer(pacc_treatment ~ time_from_bl + time_from_bl*treatment + (time_from_bl|SubjIDshort), data = data_treatment);
summary(lmm_results_habs_pacc)
plot_model(lmm_results_habs_pacc, type = c('pred'), terms = c('time_from_bl', 'treatment [0, 1]'))

# data old HAVE TO THINK MORE CAREFULLY ABOUT THIS
data_old <- subset(data_habs, data_habs$delta_all > threshold_habs)
# Assign to each subject to be sim_treatment either 0 or 1 using a random number
treatment_assignment <- data_old %>%
  distinct(SubjIDshort) %>%           # Get unique subjects
  mutate(treatment = sample(c(0, 1), n(), replace = TRUE))  # Randomly assign treatment
data_treatment <- data_old %>%
  left_join(treatment_assignment, by = 'SubjIDshort')
# Simulate 10% treatment effect on PACC
data_treatment <- data_treatment %>% 
  mutate(pacc_treatment = NP_PACC_PACC5 + abs(treatment*0.1*NP_PACC_PACC5))
