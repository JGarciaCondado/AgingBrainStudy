...
# Save path
path <- file.path(project_root, 'figures', 'longitudinal/')
...
```{r}
# Load A4 data
data_a4 <- read.csv(file.path(project_root, 'data', 'final', 'a4', 'processed', 'longitudinal.csv')) %>%
...
```{r}
# Load HABS data
data_habs <- read.csv(file.path(project_root, 'data', 'final', 'habs', 'processed', 'longitudinal.csv')) %>%
  mutate_at(vars(sex, e4_carrier, ab_status), factor)
```

```{r}
# Load ADNI data
data_adni <- read.csv(file.path(project_root, 'data', 'final', 'adni', 'processed', 'longitudinal.csv')) %>%
  mutate_at(vars(sex, e4_carrier, ab_status), factor)
...
habs_max_time <- 14 #max(data_habs$time_mri, na.rm = TRUE)
...
forest.plot.delta <- function(model_all, summary_lab, model_cu_sum) {
  forest.rma(model_all,
             slab = c("A4/LEARN", "HABS", "ADNI CU", "ADNI MCI"),
             xlab = "Standardized \u03B2", 
             header = FALSE,  # Turn off automatic header
             addpred = FALSE,
             order = c(1, 2, 3, 4),
             lwd = 2, 
             cex = 1.3, 
             xlim = c(-1.2, 0.65),
             at = seq(-0.5, 0.2, by = 0.1),
             mlab = summary_lab,
             rows = c(4, 3, 2, 1),  
             ylim = c(-3, 6.5),
             top = 2.75)
  
  # Add separator line between studies and summary estimates
  abline(h = 0, lwd = 1)
  
  # Manually add header text at desired position
  text(x = -1.2, y = 5.5, labels = "Cohort", pos = 4, cex = 1.3, font = 2)
  text(x = 0.65, y = 5.5, labels = "Estimate [95% CI]", pos = 2, cex = 1.3, font = 2)
  
  # Add CU summary polygon
  addpoly(
    x = model_cu_sum$beta,
    ci.lb = model_cu_sum$ci.lb,
    ci.ub = model_cu_sum$ci.ub,
    row = -2,
    mlab = sprintf("CU: p = %.2f, I² = %.1f%%", 
                   model_cu_sum$pval, 
                   model_cu_sum$I2),
    cex = 1.3
  )
...
# Generate label
summary_lab <- sprintf("All: p = %.2f, I² = %.1f%%", 
                       delta_sum$pval, 
                       delta_sum$I2)

# Save to file
outfile <- file.path(path, "delta_brainage_forest_new.png")
png(outfile, width = 20, height = 14, units = "cm", res = 500)
forest.plot.delta(delta_model, summary_lab, delta_cu_sum)
invisible(dev.off())  # Close the PNG device silently
...
# Save to file
outfile <- file.path(path, "ab_forest_new.png")
png(outfile, width = 20, height = 14, units = "cm", res = 500)
forest.plot.delta(ab_model, summary_lab, ab_cu_sum)
invisible(dev.off())  # Close the PNG device silently
...
# Save to file
outfile <- file.path(path, "ptau_forest_new.png")
png(outfile, width = 20, height = 14, units = "cm", res = 500)
forest.plot.delta(ptau_model, summary_lab, ptau_cu_sum)
invisible(dev.off())  # Close the PNG device silently
...
# Save to file
outfile <- file.path(path, "tau_forest_new.png")
png(outfile, width = 20, height = 14, units = "cm", res = 500)
forest.plot.delta(tau_model, summary_lab, tau_cu_sum)
invisible(dev.off())  # Close the PNG device silently
