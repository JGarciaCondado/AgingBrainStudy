...
meta_results_cu <- list(e4_cu, sex_cu, pacc_cu, ab_cu, ptau_cu, tau_cu)

# Corresponding labels for y-axis
labels <- c("APOEe4+", "Female", "PACC", "Aβ-PET", "pTau217", "Tau-PET")

# Extract estimates, confidence intervals, and p-values
estimates <- sapply(meta_results_cu, function(x) x$beta)
ci_lower <- sapply(meta_results_cu, function(x) x$ci.lb)
ci_upper <- sapply(meta_results_cu, function(x) x$ci.ub)
pvals <- sapply(meta_results_cu, function(x) x$pval)
i2 <- sapply(meta_results_cu, function(x) x$I2)
tau2 <- sapply(meta_results_cu, function(x) x$tau2)
...
# Make labels into levels
forest_data_cu$labels <- factor(forest_data_cu$labels, levels = rev(labels))

# Plot the forest plot
forest_plot_cu <- ggplot(forest_data_cu, aes(x = estimates, y = labels)) +
  geom_point(size = 1.5) +
  ...
  theme_minimal() +
  xlim(-2, 2) +
  # Add p-values below each point
  geom_text(aes(label = ifelse(pvals < 0.001, "p < 0.001", sprintf("p = %.3f", pvals))), 
          vjust = 2.5, size = 7 / .pt,
          inherit.aes = TRUE,
          check_overlap = TRUE)
...
# Print the plot
print(forest_data_cu)
print(forest_plot_cu)

# Save figure
forest_plot_cu <- forest_plot_cu + 
  graph_th
ggsave(paste(path, 'forest_plot_cu.png'), forest_plot_cu, 
...
```{r}
# List of your meta-analysis results
meta_results_mci <- list(adni_mci_e4_standrdized,
...
# Extract estimates
estimates <- sapply(meta_results_mci, function(x) x$coefficients["delta", "Estimate"])
std_errors <- sapply(meta_results_mci, function(x) x$coefficients["delta", "Std. Error"])
...
# Extract p-values
pvals <- sapply(1:length(meta_results_mci), function(i) {
  meta_results_mci[[i]]$coefficients["delta", pval_cols[i]]
})
...
forest_data_mci$labels <- factor(forest_data_mci$labels, levels = rev(labels))

# Plot the forest plot
forest_plot_mci <- ggplot(forest_data_mci, aes(x = estimates, y = labels)) +
  geom_point(size = 1.5) +
  geom_errorbarh(aes(xmin = ci_lower, xmax = ci_upper), height = 0.2) +   # 95% confidence intervals
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray") +
  labs(
...
# Print the plot
print(forest_data_mci)
print(forest_plot_mci)

# Save figure
forest_plot_mci <- forest_plot_mci + 
  graph_th
ggsave(paste(path, 'forest_plot_mci.png'), forest_plot_mci, 
       width = 80, height = 80, units = "mm", 
       dpi = 500,     # Higher resolution
       scale = 1)
```