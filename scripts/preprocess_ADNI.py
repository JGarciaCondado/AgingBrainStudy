...
# Load the dataset
df = pd.read_csv('data/ADNI/raw/adni_03192025.csv', index_col=0)
...
def normalize_mri(mri_col, icv_col):
    ...
    return adj_mri
df_baseline['bi_hippocampus'] = normalize_mri(df_baseline['bi_hippocampus'], df_baseline['MRI_FS7_rnr_ICV_vol'])
df_baseline['bi_amygdala'] = normalize_mri(df_baseline['bi_amygdala'], df_baseline['MRI_FS7_rnr_ICV_vol'])
...
covars = ['Age', 'Sex', 'Education', 'e4_carrier', 'AMYLOID_STATUS']
...
df_clinical = df_covars[['ID', 'e4_carrier', 'AMYLOID_STATUS']].copy()
df_clinical['CN'] = ((df_clinical['e4_carrier'] == 0.0) & (df_clinical['AMYLOID_STATUS'] == 0.0)).astype(int)
df_clinical['e4+'] = ((df_clinical['e4_carrier'] == 1.0) & (df_clinical['AMYLOID_STATUS'] == 0.0)).astype(int)
df_clinical['e4+ab+'] = ((df_clinical['e4_carrier'] == 1.0) & (df_clinical['AMYLOID_STATUS'] == 1.0)).astype(int)
df_clinical['ab+'] = ((df_clinical['e4_carrier'] == 0.0) & (df_clinical['AMYLOID_STATUS'] == 1.0)).astype(int)
df_clinical['unknown'] = df_clinical['AMYLOID_STATUS'].isna().astype(int)
df_clinical.drop(columns=['e4_carrier', 'AMYLOID_STATUS'], inplace=True)
